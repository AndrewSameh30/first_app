...
class PatientHeartRate extends StatefulWidget {
  //const PatientRespiratoryRate({super.key});

  @override
  State<PatientHeartRate> createState() => _HomeScreenState();
}

class _HomeScreenState extends State<PatientHeartRate> {
  ...
  void initializeReading() async {
    ...
    if (user != null) {
      ...
      if (snapshot.exists) {
        ...
        print(patientParamsSnapshotValue.values.first["HeartRate"]);
        ...
        print(last5params.map((e) => double.parse('${e["HeartRate"]}')));
        setState(() {
          _chartData = last5params
              .map<RRdata>((e) => RRdata(DateTime.parse(e["Time"]),
                  double.parse('${e["HeartRate"]}')))
                  .toList();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SfCartesianChart(
        ...
        primaryYAxis: NumericAxis(labelFormat: '{value}bpm'),
      ),
    );
  }
