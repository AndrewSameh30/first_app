...
class PatientHeight extends StatefulWidget {
  //const PatientRespiratoryRate({super.key});

  @override
  State<PatientHeight> createState() => _HomeScreenState();
}

class _HomeScreenState extends State<PatientHeight> {
  ...
  void initializeReading() async {
    ...
    if (user != null) {
      ...
      if (snapshot.exists) {
        ...
        print(patientParamsSnapshotValue.values.first["Height"]);
        ...
        print(last5params.map((e) => double.parse('${e["Height"]}')));
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SfCartesianChart(
        title: ChartTitle(text: 'Height(Last 15 records)'),
