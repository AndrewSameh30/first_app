...
class PatientSteps extends StatefulWidget {
  //const PatientRespiratoryRate({super.key});

  @override
  State<PatientSteps> createState() => _HomeScreenState();
}

class _HomeScreenState extends State<PatientSteps> {
  ...
  void initializeReading() async {
    ...
    if (user != null) {
      ...
      if (snapshot.exists) {
        ...
        print(patientParamsSnapshotValue.values.first["Steps"]);
        ...
        print(last5params.map((e) => double.parse('${e["Steps"]}')));
        setState(() {
          _chartData = last5params
              .map<RRdata>((e) => RRdata(DateTime.parse(e["Time"]),
                  double.parse('${e["Steps"]}')))
                  .toList();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SfCartesianChart(
        title: ChartTitle(text: 'No. of Steps(Last 15 records)'),
        ...
        primaryYAxis: NumericAxis(labelFormat: '{value}steps'),
      ),
    );
  }
